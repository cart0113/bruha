...
Set by `sidebar-indicator.js` in `applyActiveStates()`. Cleared and re-applied
on every `doneEach`, `hashchange`, and MutationObserver trigger:
...
| `sb-bar-level`    | `<li>` | H2 heading in sub-sidebar — direct child of outer `<ul>` (border-left indicator, margin-left: -4px)          |
| `sb-text-level`   | `<li>` | H3+ heading in sub-sidebar — inside sibling `<ul>` (border-left indicator, margin-left: -4px, extra padding) |
...
CSS effect:
`.sidebar-nav li.sb-page-collapsed > ul { display: none !important; }`
...
CSS effect:
`.sidebar-nav li.ext-folder.ext-folder-collapsed > ul { display: none; }`

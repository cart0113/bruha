...
### color-themes.css
4 color themes (parchment, pylab, blossom, midnight), each with light and dark
variants. Applied via `.theme-<name>` and `.dark-mode` classes on `<html>`.
...
button with theme swatches and dark mode toggle). Saves theme to localStorage
key `doc-theme`, dark mode to `doc-dark-mode`. Applies `.theme-*` and
`.dark-mode` classes to `<html>` before docsify renders. Icons use the SVG
sprite (icon-moon, icon-sun, icon-palette).

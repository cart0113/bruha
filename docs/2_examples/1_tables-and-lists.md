...
- **`build_sidebar(docs_folder, top_level_folders_only)`** — Generates sidebar markdown from the filesystem. Returns the content as a string.
- **`write_sidebar(docs_folder, top_level_folders_only)`** — Writes `_sidebar.md` to the docs folder. Returns the file path.
- **`load_config(docs_folder)`** — Reads `docsify-ext.yaml` and returns the config dict.
- **`generate_config_js(docs_folder)`** — Writes the generated JS config file. Returns the output path.

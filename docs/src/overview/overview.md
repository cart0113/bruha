...
- **Auto-generated sidebar from filesystem**: just add folders and markdown
  ...
  `index.html` and `_sidebar.md`. The build step is very fast (e.g. roughly 50ms
  for a 150 page site), which is useful when used as a commit hook (the
  recommended pattern). This was designed so that an LLM does not need to
  remember to update ancillary files when updating core documentation — agents
  just edit markdown and commit.

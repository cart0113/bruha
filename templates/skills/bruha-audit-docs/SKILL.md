...
Docs folders may contain symlinks to files from other repos. **Never follow,
read, edit, or audit files that resolve outside this project.** They are owned
by another repo and must not be modified here. Symlinks that resolve within this
project are fine.
...
```
.claude/skills/bruha-audit-docs/scripts/list-doc-files.sh <target-path>
```
...
Use the listing script to discover all doc files. Then **read them in the order
a reader would encounter them**:

1. Check for `_order.md` files in each folder. These control sidebar and tab
   ordering — listed items appear first, unlisted items append alphabetically.
2. If no `_order.md` exists, the file matching the folder name sorts first, then
   alphabetical.
3. Read top-level folders first (these are the navigation tabs), then drill into
   each folder following its `_order.md`.

Read the docs as a **cohesive book**, not a bag of files. Pay attention to how
one page flows into the next — does the reading order make sense? Is there a
logical progression from introduction to details to reference?
...
### Phase 4: Structure and reading order

Evaluate the docs as a cohesive book:

- **Reading order** — does the `_order.md` in each folder create a logical
  progression? Would a new user understand things in this sequence?
- **Folder hierarchy** — are pages grouped sensibly? Should anything be split
  into subfolders or merged upward? You are free to propose new folder levels
  and reorganization.
- **Navigation** — do the top-level tabs (folders) make sense as categories? Is
  anything buried that should be more prominent, or vice versa?
- **Page length** — are any pages too long (should be split) or too short
  (should be merged)?
- **Missing `_order.md`** — if a folder has no `_order.md` and the default
  alphabetical order doesn't make sense, propose one.

Propose structural changes and **ask before making them** — reorganization
affects navigation and reading flow.

### Phase 5: Quality and consistency
...
6. If you add or move pages, update `_order.md` files and run
   `docs/bin/build.sh` to regenerate the sidebar.
